...
The `bridge` service SHALL mount the host path `./data` into the container at `/var/bridge/gtfs` with the `:ro` flag, so the service can read `agency.txt`, `routes.txt`, `trips.txt`, `stops.txt`, `stop_times.txt`, `calendar.txt`, `calendar_dates.txt` (the inputs consumed by [`gtfs-static-data`](../../../specs/gtfs-static-data/spec.md)). The bridge SHALL NOT depend on the bundled `gtfs.zip` — it reads the `.txt` files directly so it doesn't need an unzip path at startup.
...
The bridge SHALL serve, on `http://bridge:3001`, the two endpoints declared in [`otp-routing`](../../../specs/otp-routing/spec.md) R-05:
