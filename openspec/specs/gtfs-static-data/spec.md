...
The static GTFS Schedule data layer for Sol Antigua urban Colonia del Sacramento. Defines the canonical `data/*.txt` files, their schemas, the fare modeling (including a placeholder mode while the operator's tariff is unconfirmed), the OSM walking graph, and the deterministic bundling + release pipeline that downstream consumers (OpenTripPlanner, MobilityDatabase, the tourist trip-planner viewer) depend on.
