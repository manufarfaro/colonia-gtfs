...
Line-schedule mode for the v0 viewer: activated by tapping a line chip in the selector or by deep-link `#line=<shortName>`, renders the full trazado (both directions + stops + live vehicles, vehicles polled every 15 s) on the map and a bottom sheet listing the day's scheduled departures grouped by direction. Builds on [`viewer-shell-and-api`](../viewer-shell-and-api/spec.md) (which exposes `/api/lines/:id` and `/api/lines/:id/vehicles`), on [`viewer-od-mode`](../viewer-od-mode/spec.md) (which provides the `useViewerMode` hook, the shared `BottomSheet` primitive, the line selector, and the previous-mode stash), and on [`viewer-stop-info-mode`](../viewer-stop-info-mode/spec.md) (tap-on-stop within the line-schedule trazado pushes the stop-info sheet and restores line-schedule on close).
