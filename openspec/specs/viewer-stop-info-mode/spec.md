...
Stop-info mode for the v0 viewer: activated by tap on a stop marker on the map or by deep-link `#stop=<gtfsId>`, renders a bottom sheet with the next arrivals (scheduled + realtime, mixed) for the named stop and polls every 30 seconds while open. Builds on [`viewer-shell-and-api`](../viewer-shell-and-api/spec.md) (which exposes `/api/stops/:id/arrivals`) and on [`viewer-od-mode`](../viewer-od-mode/spec.md) (which provides the `useViewerMode` hook and the shared `BottomSheet` primitive).
